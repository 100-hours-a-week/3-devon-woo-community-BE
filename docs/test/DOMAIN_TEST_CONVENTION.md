...
이 문서는 우리 프로젝트에서 **도메인 레벨(Entity/Policy)의 테스트가 무엇을 검증하고, 무엇은 검증하지 않을지**에 대한 기준을 정리합니다.  
서비스/리포지토리/컨트롤러 테스트와 역할을 분리해, 도메인 테스트가 **핵심 비즈니스 규칙과 불변식을 빠르게 검증하는 레이어**가 되도록 하는 것이 목적입니다.
