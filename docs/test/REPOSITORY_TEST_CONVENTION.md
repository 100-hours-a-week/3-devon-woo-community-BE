...
리포지토리 테스트는 다음을 검증합니다.

- **JPA 매핑이 의도대로 동작하는지**
  - `save` 후 `id`가 정상 발급되는지 (`PostTagRepositoryTest.saveAndFind`, `PostRepositoryTest.saveAndFind`)
  - 연관관계 필드(`@ManyToOne`, `@OneToMany`, `@ManyToMany` 등)가 올바르게 저장·조회되는지
  - 기본값/nullable 필드가 의도한 상태로 유지되는지 (`MemberRepositoryTest.saveAndFindWithEmptyAndNullFields`)
- **커스텀 쿼리 메서드가 의도대로 동작하는지**
  - `findByIdWithMember`, `findByPostIdWithTag`, `findByMemberIdWithPost` 처럼 **도메인에서 의미 있는 조회 메서드**의 결과가 기대와 일치하는지
  - 정렬/필터 조건이 올바르게 적용되는지 (예: “삭제된 게시글은 조회되지 않는다”, “본인 댓글만 조회된다” 등)
- **카운터/상태 변경용 쿼리의 부작용**
  - `incrementLikeCount`, `decrementCommentCount`, `incrementViewCount` 등의 메서드가
    - 영향을 받은 row 수를 올바르게 반환하는지,
    - 실제 엔티티의 필드 값에 반영되는지 (`PostRepositoryTest.incrementLikeCount`, `decrementLikeCount_whenZero` 등)
- **Cascade / orphanRemoval / 삭제 동작**
  - 부모 삭제 시 자식 엔티티가 적절히 삭제/보존되는지 (`PostTagRepositoryTest.cascadeDeleteWithPost`)
  - `deleteByPostId`와 같이 커스텀 삭제 메서드가 의도한 범위만 지우는지

요약하면, Repository 테스트는 **“엔티티 매핑 + 쿼리/삭제 메서드 + 카운터 쿼리”**가 실제 DB 위에서 우리가 설계한 대로 동작하는지에만 집중합니다.
...
## 6. 언제 Repository 테스트를 추가/수정할까?

- **새로운 커스텀 쿼리 메서드를 추가했을 때**
  - `findBy*` / `deleteBy*` / `existsBy*` 등 도메인적으로 의미 있는 메서드는  
    최소 1개의 Repository 테스트를 갖도록 한다.
- **엔티티에 중요한 필드/연관관계가 추가되었을 때**
  - `saveAndFind*` 또는 관련 테스트에 필드를 추가해, 저장/조회가 깨지지 않았는지 검증한다.
- **카운트/상태 변경 쿼리를 JPQL/Querydsl로 새로 정의했을 때**
  - “정상 증가/감소 + 0 경계값” 정도는 Repository 테스트에 추가한다.
- **Cascade 설정을 바꾸었을 때**
  - 최소 한 개의 cascade 관련 테스트를 추가/수정해, 삭제 동작이 기대대로 유지되는지 확인한다.

---

## 7. 요약

- Repository 테스트는 **JPA 매핑·커스텀 쿼리·카운터/삭제 쿼리·Cascade 동작**을 실제 DB 위에서 검증하는 레이어다.
- 서비스/정책/컨트롤러 수준의 비즈니스 규칙은 다른 테스트 레이어에서 담당하고, Repository 테스트는 그 기반이 되는 **“데이터 접근 계약”**만 보장한다.
- 테스트는 “쿼리 구현 상세”가 아니라 “도메인 규칙을 만족하는 결과”를 검증해야 하며,  
  이름과 시나리오도 그 규칙을 설명하는 문서 역할을 하도록 작성한다.
