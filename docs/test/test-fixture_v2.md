...
> 대상: 전체 테스트 레이어에서 사용하는 Test Fixture  
> 참고: `DOMAIN_TEST_CONVENTION.md`, `REPOSITORY_TEST_CONVENTION.md`, `SERVICE_TEST_CONVENTION.md`, `CONTROLLER_WEBMVC_TEST_CONVENTION.md`, `INTEGRATION_TEST_CONVENTION.md`

이 문서는 이유·배경을 다루지 않고, Test Fixture를 정의·사용할 때 따라야 하는 **규칙만** 정리합니다.
...
- Test Fixture는 **테스트에서 반복되는 유효한 기본 상태**를 캡슐화하는 도구입니다.
- 각 레이어의 역할(도메인/Repository/Service/Controller/통합 테스트)에 맞게,
  - 어디까지 Fixture를 쓰고,
  - 어디서부터는 실제 팩토리/DTO/응답을 직접 검증할지 위 규칙을 따릅니다. 
